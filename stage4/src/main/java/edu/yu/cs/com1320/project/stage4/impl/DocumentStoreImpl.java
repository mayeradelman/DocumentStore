...
public class DocumentStoreImpl implements DocumentStore {
	...
	public List<String> searchByPrefix(String prefix){
		List<Document> documents = this.trie.getAllWithPrefixSorted(prefix, this.generateKeywordPrefixComparator(prefix));
		List<String> documentsAsTexts = new LinkedList<>();
		long lastUsedTime = System.nanoTime();
		for(Document document : documents){
			document.setLastUseTime(lastUsedTime);
	...
	public Set<URI> deleteAllWithPrefix(String keywordPrefix) {
		Set<Document> documentsToDelete = new HashSet<>(this.trie.getAllWithPrefixSorted(keywordPrefix, this.generateKeywordPrefixComparator(keywordPrefix)));
		Set<URI> deletedUris = new HashSet<>();
		CommandSet<URI> commandSet = new CommandSet<>();
		if(!documentsToDelete.isEmpty()) {
	...
	private void implementOverride(DocumentImpl newDocument, DocumentImpl previousDocument){
		...
		this.minHeap.reHeapify(previousDocument);
	...
	private void removeExtraDocuments(){
		while(this.documentCount > maxDocumentCount || this.byteCount > this.maxDocumentBytes){
			DocumentImpl documentToPurge = (DocumentImpl) this.minHeap.removeMin();
			this.hashTable.put(documentToPurge.getKey(), null);
			for(String key : documentToPurge.getWords()) {
				this.trie.delete(key, documentToPurge);
			}
			StackImpl<Undoable> tempStack = new StackImpl<>();
			while(this.commandStack.peek() != null){
				Undoable command = this.commandStack.pop();
				if(command instanceof  GenericCommand){
					if(!((GenericCommand<URI>) command).getTarget().equals(documentToPurge.getKey())){
						tempStack.push(command);
					}
					continue;
				}
				if(command instanceof CommandSet){
					if(((CommandSet<URI>) command).containsTarget(documentToPurge.getKey())) {
						CommandSet<URI> commandSet = (CommandSet<URI>) command;
						Iterator<GenericCommand<URI>> commandIterator = commandSet.iterator();
						while (commandIterator.hasNext()) {
							if (commandIterator.next().getTarget().equals(documentToPurge.getKey())) {
			...
			this.documentCount--;
			this.byteCount = this.byteCount - documentToPurge.byteUsage();
		}
	}
}